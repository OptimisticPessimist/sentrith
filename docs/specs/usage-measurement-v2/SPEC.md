...
- baseline labeling via one environment variable
...
- REQ-004: HEAD is captured at `UserPromptSubmit` and compared at `Stop`; a change marks the turn as the commit-closing row.
...
- REQ-007: `SENTRITH_PHASE` environment variable supplies the phase when `--phase` is not given; precedence is `--phase` > env > `standard`.
- REQ-008: `usage report --tasks` groups turn rows into tasks by session and head_sha transitions and reports task-level stats; success rate counts `yes/(yes+no)` and reports the unknown share separately.
...
- [ ] AC-001: Given a fixture transcript window with duplicated message ids, token sums count each message id once (unit test).
- [ ] AC-002: A fixture window containing a failing test command yields `verification=fail`; a passing one yields `pass` (unit test).
- [ ] AC-003: A v1 CSV is migrated to v2 with padded columns and all v1 data preserved (unit test).
- [ ] AC-004: Phase resolution honors `--phase` > `SENTRITH_PHASE` > default (unit test).
- [ ] AC-005: Task grouping splits rows on head_sha transitions and success rate excludes `unknown` from the denominator (unit test).
- [ ] AC-006: `cargo test` passes in CI on Linux/Windows/macOS.
